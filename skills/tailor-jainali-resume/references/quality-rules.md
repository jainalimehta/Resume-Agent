...
- Choose three to five role-specific fit themes and front-load them.
- Use compact labelled bullets that identify the evidence before the explanation.
- Prefer action + scope + method + verified result/purpose. Omit a result when none is verified.
...
- Use US Letter, one column, standard headings, embedded searchable fonts, and black body text.
- Keep essential contact details in the document body through the two-row header, never a PDF header/footer.
- Do not use icons, photos, graphics, rating bars, text boxes, columns, sidebars, charts, or skill bars.
- Keep visible, useful hyperlinks and confirm header-first `pdftotext` reading order.
- ATS is the correct acronym; never call the document ATDS-supported.
...
Keep Education and Credentials separate. Require exact published project titles: `E-Commerce Sales Analytics`, `Healthcare Patient & Hospital Analytics`, and `HR Analytics`.

## Cover Letter

- Use the matching two-row letterhead, thin rule, current date, and centered `RE:` subject.
- Target 300--425 words in four to six short paragraphs.
- Use two evidence-led paragraphs plus collaboration/learning context when relevant.
- Include employer-specific motivation and a direct close.
- Use a verified recipient name or `Dear Hiring Manager,`.
- Use a plain-text signature name. Never fabricate a signature image.
